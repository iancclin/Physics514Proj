...
% V = @(x) (0.5*x.^2);
plot(xL:0.01:xR,V(xL:0.01:xR),'k-');
hold on;
    ...
    while iter <= maxIter
        ...
        elseif signChange < state
            ...
            if abs(diffVal) < tol
                ...
                positionTailL = -(10000*hL:-hL:hL);
                positionTailL = gridL(1)+positionTailL;
                positionTailR = hR:hR:(10000*hR);
                positionTailR = positionTailR++gridR(end);
                position(state+1,:) = [positionTailL gridL gridR(2:end) positionTailR];
                psiTailL = exp(-(0.5-((10000*hL):-hL:hL)));
                psiTailR = exp(-(0.5-(hR:hR:(10000*hR))));
                psiR = [psiR psiTailR];
                psiL = [psiTailL psiL];
                psiR = psiR*(psiL(end)/psiR(1));
                intL = hL*sum(0.5*(psiL(1:end-1).^2+psiL(2:end).^2));
                intR = hR*sum(0.5*(psiR(1:end-1).^2+psiR(2:end).^2));
                normPsi = sqrt(intL+intR);
...
%     end
    plot(position(state+1,:),10*psi(state+1,:)+energy(state+1));
end
axis([0 1 -250 0]);
xlabel('x');
ylabel('energy');
legend('ground state','1st excited state','2nd excited state','3rd excited state','4th excited state');
